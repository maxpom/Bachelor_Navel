// Based on implementation by mmcza (2024)
// Original: https://github.com/mmcza/TurtleBot-RRT-Star
// License: Apache-2.0
// Adapted for Navel Trajectory by Maximilian Pomplun
//
// Modifications for ROS2 Jazzy compatibility:
// - Added std::function<bool()> cancel_checker parameter to createPlan()
// - Fixed member initialization order in Vertex constructor (warning removal)


#ifndef RRTSTAR_PLANNER__RRTSTAR_PLANNER_HPP_
#define RRTSTAR_PLANNER__RRTSTAR_PLANNER_HPP_
...
struct Vertex {
    ...
    Vertex(double x_val, double y_val, Vertex* p = nullptr, double travel_distance = 0) :
        x(x_val), y(y_val), cost(travel_distance), parent(p) {}
};
